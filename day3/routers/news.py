...
async def get_categories(db: AsyncSession = Depends(get_db), skill: int = 0, limit: int = 100):
    categories = await news.get_categories(db, skill, limit)
    return {
        "code": 200,
        "message": "新闻分类获取成功",
        "data": categories
...
async def get_news_detail(
    ...
    related_news = await news.get_related_news(db, news_detail.category_id, news_id, limit=5)
    return {
      "code": 200,
      "message": "success",
      "data": {
